...
$debug_info = [];
$log_file = __DIR__ . '/logs/obter-placar-mes-' . date('Y-m-d') . '.log';
...
log_debug("==== INICIANDO OBTER-PLACAR-MES ====");
...
try {
    ...
    log_debug("Período solicitado: $primeiro_dia_mes a $ultimo_dia_mes para usuario_id=$id_usuario");
    
    // 🔍 Buscar TODAS as mensagens do mês INTEIRO com título, resultado E ODDS (como faz o bloco 1)
    // NOTA: Sem filtro de id_usuario, igual ao obter-placar-dia.php
    ...
    // 📊 Inicializar array com as 3 apostas (EXATAMENTE COMO BLOCO 1)
    $apostas = [
        '+1⚽GOL' => [
            'green' => 0, 
            'red' => 0,
    ...
    // 🔄 Processar cada mensagem (MESMO ALGORITMO DO BLOCO 1)
    ...
    // 🎯 Calcular totais gerais (MESMO MÉTODO DO BLOCO 1)
    $total_green = 0;
    $total_red = 0;
    ...
    // 🎯 Retornar dados em JSON (COMPATÍVEL COM BLOCO 1)
    echo json_encode([
        'success' => true,
        'apostas' => [
            'aposta_1' => [
        ...
        'informacoes' => [
            ...
            'descricao' => 'Agregação de todos os dias do mês com mesmo método que Bloco 1'
        ],
        'message' => "Placar do mês: $total_green Green, $total_red Red"
    ]);
